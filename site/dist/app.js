...
function detail(p){
 ...
 $('detail').innerHTML=`<button class="mobile-back" id="back"><span aria-hidden="true">←</span> Back to programs</button><div class="detail-top"><div class="detail-company"><span class="monogram">${escape(p.initials)}</span><div><div class="company">${escape(p.company)}</div><div class="row-country">${escape(p.country)}</div></div>${badge(p)}</div><h2>${escape(p.title)}</h2><div class="detail-location">${escape(p.location)}</div><div class="detail-tags">${tags(p)}${p.areas.map(a=>`<span class="tag">${escape(a)}</span>`).join('')}</div><a class="apply" href="${escape(p.source)}" target="_blank" rel="noopener noreferrer">${state.status==='open'?'Apply on employer website':'View official program'}<span aria-hidden="true">↗</span></a><p class="source-caption">${state.status==='open'?'Opens the official job posting in a new tab':'Check current openings and full requirements with the employer'}</p></div><div class="detail-body"><dl class="facts"><div><dt>Employment starts</dt><dd>${startLabel(p)}</dd></div><div><dt>Program length</dt><dd>${escape(p.duration||'Not announced')}</dd></div><div><dt>Degree eligibility</dt><dd>${degrees(p)}</dd></div><div><dt>${state.status==='closed'?'Applications closed':'Application deadline'}</dt><dd>${state.closure?dateLabel(state.closure):p.deadline?dateLabel(p.deadline):'Not announced'}</dd></div></dl><h3>About the program</h3><p>${escape(p.summary)}</p><h3>What to expect</h3><p>${escape(p.structure)}</p><h3>Who can apply</h3><p>${escape(p.eligibility)}</p>${p.notes?`<h3>Before you apply</h3><p>${escape(p.notes)}</p>`:''}${state.status==='closed'?'<p class="muted">This opening has closed. It will leave the directory 14 days after its recorded closing date.</p>':''}<div class="verification">✓ Official source reviewed · ${dateLabel(p.checked)}<a href="${escape(p.source)}" target="_blank" rel="noopener noreferrer">${escape(new URL(p.source).hostname)} ↗</a></div></div>`;
...
function render(){
 ...
 $('program-list').innerHTML=visible.length?visible.map(p=>`<button class="program-row ${p.id===selected?'selected':''}" data-id="${p.id}" aria-pressed="${p.id===selected}" aria-controls="detail"><div class="row-top"><span class="monogram" aria-hidden="true">${escape(p.initials)}</span><div><div class="company">${escape(p.company)}</div><div class="row-country">${escape(p.country)}</div></div>${badge(p)}</div><h2>${escape(p.title)}</h2><div class="row-meta">${escape(p.areas.join(' · '))}</div><div class="row-bottom"><span>${tags(p)}</span><span class="row-start">Starts ${startLabel(p)}</span></div><span class="row-arrow" aria-hidden="true">›</span></button>`).join(''):'<div class="empty"><h2>No programs match just yet.</h2><p>Try another business area or include programs<br>whose start date hasn’t been announced.</p><button id="empty-reset">Clear all filters</button></div>';
